...
class GameMap:
	...
	def place_entities(self, room, entities):
		...
		equipment_stream = open(os.path.join(definitions.ROOT_DIR,'data','objects','equipment.yaml'), 'r')
		equipment_list = yaml.load(equipment_stream)
		item_chances = []
		item_index = 0
		for i in item_list:
			item_chances.append(['item', from_dungeon_level(item_list[i].get('loot_chance'), self.dungeon_level), i])
			item_index += 1		

		for i in equipment_list:
			item_chances.append(['equipment',from_dungeon_level(equipment_list[i].get('loot_chance'), self.dungeon_level), i])
			item_index += 1
